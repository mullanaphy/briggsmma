<?php
    ...
    class Admin extends AController
    {
        ...
        public function __construct(App $app = null)
        {
            ...
            if (!$user->exists()) {
                ...
                if (!$authorize->exists()) {
                    ...
                    $authorize->allow = 'instructor admin super-admin';
                    $authorize->deny = 'all';
        ...
        public function instructor_get()
        {
            ...
            if ($id !== false) {
                if ($id) {
                    ...
                    $item = new User($request->get('instructor', []));
                }
                $content->setTemplate('admin/instructor/item.phtml');
                $content->setVariable('item', $item);
                $breadcrumb = $layout->block('breadcrumb');
                $breadcrumb->setVariable('item', $item);
            } else {
                ...
                $content->setTemplate('admin/instructor/collection.phtml');
                $content->setVariable('collection', $collection);
                $content->setChild('pagination', [
                    'viewClass' => 'pagination',
                    'pageId' => $pageId,
                    ...
                    'total' => $collection->count(),
                    'url' => [
                        $this->url('admin/instructor'),
                        'limit' => $limit
                    ]
                ]);
            }
            if ($message = $app->get('session/admin/instructor/message')) {
                $app->delete('session/admin/instructor/message');
                $message['template'] = 'generic/message.phtml';
                $content->setChild('message', $message);
            }
        }
         ...
         */
        public function instructor_post()
        {
            $app = $this->getApp();
            $request = $this->getRequest();
            ...
            try {
                ...
                if ($id) {
                    $item = $manager->load($id, new User);
                    if (!$item->exists() || $item->deleted) {
                        $app->set('session/admin/instructor/message', [
                            'title' => 'Seriously?',
                            'type' => 'warning',
                            'message' => 'No instructor found for id: ' . $id
                        ]);
                        return $this->redirect('/admin/instructor');
                    }
                    $data['updated'] = $datetime;
                } else {
                    $item = new User($data);
                    if (!$data['password']) {
                        throw new \InvalidArgumentException('You must provide a password for new instructors.');
                ...
                $manager->save($item);
                return $this->renderResponse('instructor', [
                    'title' => 'Great Success!',
                    'type' => 'success',
         ...
         */
        public function instructor_put()
        {
            $this->instructor_post();
        }

        /**
         * DELETE /admin/instructor/id/{id}
         */
        public function instructor_delete()
        {
            $app = $this->getApp();
            $request = $this->getRequest();
        ...
        {
            $statusCode = $message['type'] === 'success'
                ? 200
                : 500;
